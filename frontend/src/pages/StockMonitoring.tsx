          ...
          <div className="space-y-3">
            {lowStockItems.length > 0 ? (
              lowStockItems.map((product) => (
                <div key={`low-stock-${product.id}`} className="flex justify-between items-center p-3 hover:bg-gray-50 rounded">
                  <span className="text-sm font-medium">{product.name}</span>
                  <span className={`px-3 py-1 text-xs font-medium rounded-full ${
                    product.stock === 0 
                      ? 'bg-red-100 text-red-800' 
                      : product.stock <= (product.low_stock_threshold * 0.5)
                        ? 'bg-red-100 text-red-800'
                        : 'bg-yellow-100 text-yellow-800'
                  }`}>
                    {product.stock === 0 ? 'Out of Stock' : `${product.stock} left (Threshold: ${product.low_stock_threshold})`}
                  </span>
                </div>
              ))
            ) : (
